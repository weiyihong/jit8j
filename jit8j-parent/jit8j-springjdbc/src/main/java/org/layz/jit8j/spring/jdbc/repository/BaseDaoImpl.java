package org.layz.jit8j.spring.jdbc.repository;
...
import org.layz.jit8j.spring.jdbc.factory.RowMapperFactory;
import org.layz.jit8j.spring.jdbc.factory.SqlBuildFactory;
...
public class BaseDaoImpl<T> extends JdbcDaoSupport implements BaseDao<T> {
	...
	public T findById(Long id) {
		try {
			if(null == id) {
				throw new Jit8jJdbcRuntimeException("请选择一条记录");
			}
	...
	public List<T> queryByEntity(T t) {
		return queryByEntity(t, null);
	}
	
	@Override
	public List<T> queryByEntity(T t, String orderBy) {
		SqlParam sqlParam = sqlBuildFactory.buildSql(clazz, QUERY_BY_ENTITY, t, new Pageable(orderBy));
		RowMapper<T> rowMapper = rowMapperFactory.getRowMapper(clazz);
		LOGGER.debug("parmeter args: {}", Arrays.toString(sqlParam.getArgs()));
	...
	public Page<T> queryPage(T t, Pageable pageable){
		...
		if(total <= pageable.getOffset()) {
			...
			SqlParam dataSqlParam = sqlBuildFactory.buildSql(clazz, QUERY_BY_ENTITY, t, pageable);
	...
	protected void initTemplateConfig() {
		...
		if(null != type) {
			...
			throw new Jit8jJdbcRuntimeException("类型解析异常");
		}
	}
	/**
	 * @param pageable
	...
	private Type getParameterizedType(Type type){
        if(type instanceof Class) {
            ...
            if(typeName.startsWith("org.layz.jit8j.spring.jdbc.repository.BaseDaoImpl<")){
                return type;
            }
